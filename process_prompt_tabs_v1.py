...
def calculate_clip(input_text, max_tokens=77):
    ...
    tokens = clip.tokenize(input_text)

    if tokens.shape[1] > max_tokens:
        print(f"Warning: Input text has too many tokens and will be truncated to {max_tokens} tokens.")
...
def calculate_umap(embeddings, n_neighbors):
    #print("Calculating umap embeddings for:", embeddings)
    print("Calculating UMAP, n_neighbors = ", n_neighbors)
...
def update_output(n_clicks, radar_metrics, heatmap_metrics, pair_metrics, selected_rows, article_id, value, current_row_data):
    global all_outputs
