...
n_articles = 3
current_article = 0
for article in cnn_dailymail_data['train']:
    current_article += 1
    if current_article > n_articles:
        break
...
n_paragraphs = 1
n_qas = 1
for test_sample in quac_dataset['test'][:n_samples]:
    for paragraph in test_sample['paragraphs'][:n_paragraphs]:

    ground_truth_data[test_sample['question']] = {
        'output': test_sample['annotations'][0]['short_answers'][0]['text'],
...
#model_name = 't5-large'
#gpt_tokenizer = T5Tokenizer.from_pretrained(model_name)
#gpt_model = T5ForConditionalGeneration.from_pretrained(model_name)
# Load the Longformer model and tokenizer
#model_name = 'allenai/longformer-base-4096'
#longformer_tokenizer = LongformerTokenizer.from_pretrained(model_name)
#longformer_model = LongformerForSequenceClassification.from_pretrained(model_name)

model_name = 'facebook/bart-large'
bart_tokenizer = BartTokenizer.from_pretrained(model_name)
bart_model = BartForConditionalGeneration.from_pretrained(model_name)
gpt_model = bart_model
gpt_tokenizer = bart_tokenizer
...
def process_prompt(prompt):
    outputs = {}
    max_length = 1024  # typically 1024 for this model
    for question, data in ground_truth_data.items():
        context = data["context"]
        full_prompt = f"prompt: {prompt} question: {question} context: {context}"
        start_time = time.time()
        
        # Tokenize and generate response
        input_ids = gpt_tokenizer.encode(full_prompt, return_tensors='pt')
        if input_ids.shape[1] > max_length:
            print(f"Warning: Input length ({input_ids.shape[1]}) exceeds model maximum ({max_length}). Truncating input.")
            input_ids = input_ids[:, :max_length]

        output = gpt_model.generate(input_ids, max_length=150, num_beams=4, early_stopping=True)
...
def process_prompt(prompt):
    ...
    for question, data in ground_truth_data.items():
        context = data["context"]
        full_prompt = f"{prompt}. context: {context}"
        start_time = time.time()
        result = summarization_pipeline(full_prompt)
        end_time = time.time()
        response_time = end_time - start_time
        outputs[question] = {
            'answer': result[0]['summary_text'],
            'response_time': response_time,
            'confidence': 0  # Abstractive summarization models do not provide confidence scores
        }
    return outputs
...
def process_prompt(prompt):
    ...
    for question, data in ground_truth_data.items():
        context = data["context"]
        full_prompt = f"context: {context} question: {question} prompt: {prompt}"
        ...
        outputs[question] = {
            'answer': result[0]['summary_text'],
            'response_time': response_time,
            'confidence': 0  # T5 doesn't provide a confidence score directly
        }
    return outputs'''
